      ...
      steem.api.getAccounts([username], async (err, acc) => {
        ...
        if (acc[0].json_metadata) {
          if (JSON.parse(acc[0].json_metadata)["profile"]) {
            imageUrl = JSON.parse(acc[0].json_metadata)["profile"][
              "profile_image"
            ];
          }
        }
    ...
    return new Promise(resolve => {
      steem.api.getAccounts([username], async (err, acc) => {
        ...
        if (acc.length > 0) {
          ...
          const { posting } = steem.auth.getPrivateKeys(username, password, [
            "posting"
          ]);
    ...
    return new Promise(async resolve => {
      try {
        ...
        if (currentlyLoggedInUser == username) {
          var objOfPermLinksOfPostsUsersMade = await firebase.getListOfPermLinks(
            username,
            "postsAboutBooks"
          );
          var listOfPermLinksPosts = [];
          for (var elem in objOfPermLinksOfPostsUsersMade) {
            listOfPermLinksPosts =
              objOfPermLinksOfPostsUsersMade[elem][username];
          }
          var listOfPostUsersMade = await this.getListOfPostsByUser(
            listOfPermLinksPosts,
            username
          );
          var listOfPostsOfBookmarks = [];
          var listOfPermLinksBookmarks = [];
          var objOfBookmarkedPermLinks = await firebase.getListOfPermLinks(
            username,
            "bookmarks"
          );
          for (var user in objOfBookmarkedPermLinks) {
            ...
            var posts = await this.getListOfPostsByUser(
              listOfPermLinksBookmarks,
              user
            );
